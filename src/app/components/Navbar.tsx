...
export default function Navbar() {
  ...
  const hiddenRoutes = ['/createresume', '/uploade'];

  // If current route is in hiddenRoutes, don't render the navbar
  ...
  return (
    <div className="navbar bg-base-100 shadow-sm relative z-50 ">
      ...
      <div className="navbar-end hidden lg:flex m-2 gap-2">
        <Link href="/login" className="text-blue-700">Sign in</Link>
        <Link href="/login" className="bg-blue-700 text-white h-10 p-2 rounded">Create my Resume</Link>
      </div>
      ...
      {/* Mobile Dropdown */}
      {menuOpen && (
        <div className="absolute top-full left-0 w-full bg-base-100 shadow-md lg:hidden p-4">
          <ul className="flex flex-col space-y-2">
            ...
            <li><Link href="/login" className="text-blue-700" onClick={() => setMenuOpen(false)}>Sign in</Link></li>
            <li>
              <Link
                href="/login"
                className="bg-blue-700 text-white h-10 p-2 rounded"
